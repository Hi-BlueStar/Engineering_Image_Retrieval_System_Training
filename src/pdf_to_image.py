"""PDF 轉影像工具（支援多執行緒、資料集分層切分、Rich 進度顯示）。

此模組專為機器學習（ML）與電腦視覺（CV）的資料集前處理而設計：

- 遞迴掃描根目錄 `root_dir`，以父資料夾名稱作為類別標籤（class label）。
- 將每份 PDF 的每一頁視為獨立影像，轉為高解析度 PNG。
- 先以每頁為單位進行分層抽樣（stratified split），再依 split（train/val）及輸出模式（flat / nested）將影像寫入對應位置。
...
- 當 `flat_output=True` 時，實際輸出資料夾名稱會自動加上 `_flat` 後綴。
...
def _validate_inputs(root_dir: Path, output_dir: Path, dpi: int, val_split: float) -> None:
    ...
    Args:
        ...
        output_dir: 預期輸出根目錄（實際可能依 `flat_output` 加上 `_flat` 後綴）。
        dpi: 轉檔解析度（DPI）。
        val_split: 驗證集比例（0~1 之間）。

    Raises:
        FileNotFoundError: 當 `root_dir` 不存在時。
        ValueError: 當 `dpi` 或 `val_split` 不合規時。
    """
...
def _plan_tasks(
    ...
    2. 依 `class_label` 進行分層切分（stratified split），決定每頁屬於 'train' 或 'val'。
    3. 依 `flat_output` 與 `split` 設定各頁影像的輸出相對路徑。
    4. 以 `source_pdf` 分組，回傳供多執行緒逐檔處理。

    Args:
        pdfs_with_labels: (pdf_path, class_label) 之列表。
        root_dir: 根目錄（用於計算來源相對路徑時可能會用到）。
        effective_output_dir: 實際輸出根目錄（flat 模式可能加上 `_flat`）。
        dpi: 轉檔 DPI（此處不直接用於規劃，但保留參數以利擴充）。
        val_split: 驗證集比例（0~1）。
        flat_output: 是否使用 flat 模式輸出。
        random_state: 隨機種子（確保切分可重現）。
    ...
    for class_label, items in by_class.items():
        ...
        for idx, (pdf_path, cls, page_index) in enumerate(items_copy):
            ...
            if flat_output:
                # flat: 檔名帶入類別與檔名（避免名稱衝突）
                fname = (
                    f"{_sanitize_component_for_filename(cls)}__"
        ...
        try:
            for t in tasks:
                ...
                try:
                    ...
                    dest_abs = effective_output_dir / t.dest_rel_path
...
def run(
    ...
    """執行 PDF 轉影像並輸出/返回 manifest。

    本函式可作為模組介面呼叫，完成掃描、分層切分、並行轉換、輸出影像與
    `manifest.csv`，同時回傳 pandas.DataFrame 結果。

    Args:
        root_dir: 根目錄路徑；其下為類別資料夾，每個資料夾包含若幾 PDF。
        output_dir: 輸出根目錄；若 `flat_output=True`，實際會加上 `_flat` 後綴。
        dpi: 影像輸出解析度（透過縮放控制渲染倍率）。
        flat_output: 是否採用 flat 輸出模式（不建立類別子資料夾）。
        val_split: 驗證集比例（分層切分，依類別獨立比例切分）。
        random_state: 隨機種子（確保切分可重現）。
        max_workers: 最大執行緒數；預設使用 `os.cpu_count()` 合理值。

    Returns:
        pandas.DataFrame: 含欄位 `source_pdf`, `class_label`, `image_path`, `split`。

    Raises:
        FileNotFoundError: 當 `root_dir` 無法存取時。
        ValueError: 當 `dpi` 或 `val_split` 無效時。
    """
    ...
    _validate_inputs(root_dir, output_dir, dpi, val_split)

    # flat 模式之實際輸出資料夾名稱規則：附加 _flat 後綴
    effective_output_dir = output_dir
    if flat_output:
        effective_output_dir = output_dir.with_name(output_dir.name + "_flat")
    ...
    console.print(
        ...
        f"輸出目錄: [green]{effective_output_dir}[/green]\n"
        f"模式: {'[magenta]flat[/magenta]' if flat_output else '[magenta]nested[/magenta]'}\n"
        f"DPI: [yellow]{dpi}[/yellow], 分割: [yellow]{(1.0 - val_split):.0%} train / {val_split:.0%} val[/yellow]\n"
...
if __name__ == "__main__":
    ...
    #         output_dir=r"./data/engineering_images_600dpi",          # 輸出根目錄（flat 模式將自動加 _flat）
    #         dpi=600,                          # 影像解析度（DPI）
    #         flat_output=False,                # True -> 不建立類別子資料夾，檔名帶類別
    #         val_split=0.2,                    # 驗證集比例
    #         random_state=42,                  # 隨機種子
    #         max_workers=8,                 # 預設為 CPU 合理值
    #     )
...
"""
uv run python src/pdf_to_image3.py --root_dir ./data/吉輔提供資料 --output_dir ./data/engineering_images_100dpi --dpi 100 --val_split 0.2 --random_state 42

uv run python src/pdf_to_image3.py --root_dir ./data/吉輔提供資料 --output_dir ./data/engineering_images_100dpi --dpi 100 --flat_output --val_split 0.2 --random_state 42

uv run python src/pdf_to_image3.py --root_dir ./data/吉輔提供資料 --output_dir ./data/engineering_images_200dpi --dpi 200 --val_split 0.2 --random_state 42

uv run python src/pdf_to_image3.py --root_dir ./data/吉輔提供資料 --output_dir ./data/engineering_images_200dpi --dpi 200 --flat_output --val_split 0.2 --random_state 42

uv run python src/pdf_to_image3.py --root_dir ./data/吉輔提供資料 --output_dir ./data/engineering_images_400dpi --dpi 400 --val_split 0.2 --random_state 42

uv run python src/pdf_to_image3.py --root_dir ./data/吉輔提供資料 --output_dir ./data/engineering_images_400dpi --dpi 400 --flat_output --val_split 0.2 --random_state 42

uv run python src/pdf_to_image3.py --root_dir ./data/吉輔提供資料 --output_dir ./data/engineering_images_600dpi --dpi 600 --val_split 0.2 --random_state 42
