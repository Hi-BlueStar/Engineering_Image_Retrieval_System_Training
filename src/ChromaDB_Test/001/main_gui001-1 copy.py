...
from PySide6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout, 
                               ...
                               QProgressBar, QTabWidget, QGraphicsOpacityEffect,
                               QSizePolicy, QPlainTextEdit)
from PySide6.QtCore import (Qt, QThread, Signal, QSize, QPropertyAnimation,
                            QEasingCurve, QTimer, QRect, QPoint, QMimeData, Property)
from PySide6.QtGui import (QPixmap, QColor, QFont, QIcon, QPainter, QPen, 
                           QBrush, QPolygon, QPainterPath, QDragEnterEvent, QDropEvent)
...
class EvaButton(QPushButton):
    ...
    def paintEvent(self, event):
        ...
        current_color = QColor(NERV.COLOR_ACCENT) if is_hover else QColor(self.base_color)
...
class DropZone(QLabel):
    ...
    def dragEnterEvent(self, event: QDragEnterEvent):
        if event.mimeData().hasUrls():
            ...
            self.setStyleSheet(f"border: 2px solid {NERV.COLOR_ACCENT}; color: {NERV.COLOR_ACCENT};")
        else:
            event.ignore()
...
class ResultWidget(EvaFrame):
    ...
    def __init__(self, data):
        ...
        self.img_lbl.setStyleSheet(f"border: 1px solid {NERV.COLOR_MAIN}; background: #000;")
        
        pix = QPixmap(data['uri'])
        if not pix.isNull():
            # 保持比例縮放
            self.img_lbl.setPixmap(pix.scaled(200, 140, Qt.KeepAspectRatio, Qt.SmoothTransformation))
        else:
        ...
        info = QLabel(f"SYNC RATE: {sync_rate:.1f}%\nID: {data['meta'].get('filename', 'UNK')[:15]}...")
        info.setStyleSheet(f"color: {color}; font-family: '{NERV.FONT_TECH}'; font-size: 11px;")
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        side_panel.setStyleSheet(f"background: rgba(20, 20, 20, 0.9); border-right: 2px solid {NERV.COLOR_MAIN};")
        side_layout = QVBoxLayout(side_panel)
        side_layout.setContentsMargins(20, 20, 20, 20)
        
        # 1. Header
        header = QLabel("MAGI\nSYSTEM")
        header.setStyleSheet(f"font-family: '{NERV.FONT_HEADER}'; font-size: 48px; color: {NERV.COLOR_MAIN};")
        ...
        self.import_status.setStyleSheet(f"color: {NERV.COLOR_MAIN}; font-family: '{NERV.FONT_TECH}';")
        ...
        self.log_box.setStyleSheet(f"background: transparent; border: 1px dashed {NERV.COLOR_MAIN}; color: {NERV.COLOR_MAIN}; font-family: '{NERV.FONT_TECH}'; font-size: 10px;")
        ...
        self.status_lbl.setStyleSheet(f"font-family: '{NERV.FONT_HEADER}'; font-size: 24px; color: {NERV.COLOR_MAIN};")
    ...
    def log_status(self, level, msg):
        ...
        color = NERV.COLOR_DANGER if level == "ERROR" else NERV.COLOR_ACCENT if level == "SUCCESS" else NERV.COLOR_MAIN
        ...
        if level == "ERROR":
            self.status_lbl.setStyleSheet(f"font-family: '{NERV.FONT_HEADER}'; font-size: 24px; color: {NERV.COLOR_DANGER};")
        else:
             self.status_lbl.setStyleSheet(f"font-family: '{NERV.FONT_HEADER}'; font-size: 24px; color: {NERV.COLOR_MAIN};")
    ...
    def on_image_search(self):
        if not hasattr(self, 'current_img_path'): return
        self.reset_results()
    ...
    def load_more(self):
        if not self.last_query_type: return
