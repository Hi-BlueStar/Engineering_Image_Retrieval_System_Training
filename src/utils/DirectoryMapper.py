...
class DirectoryMapper:
    ...
    def generate_tree_text(self, current_path: Path = None, prefix: str = "") -> str:
        ...
        try:
            ...
            for i, item in enumerate(items):
                ...
                output += f"{prefix}{connector}{item.name}{'/' if item.is_dir() else ''}\n"
    ...
    def generate_json_structure(self, current_path: Path = None) -> Dict:
        ...
        if current_path.is_dir():
            children = []
            try:
                items = sorted(list(current_path.iterdir()), key=lambda x: x.name.lower())
                for item in items:
    ...
    def save_to_file(self, output_format: str = 'txt', output_filename: str = 'structure_output'):
        """
        將結果寫入檔案的統一接口。
        """
