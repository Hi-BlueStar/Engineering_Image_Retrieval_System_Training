...
from v2.src.model.simsiam import SimSiam
...
def main():
    ...
    checkpoint_path = st.sidebar.text_input("自定義 Checkpoint 路徑 (可留空)", "")
    
    n_seeds = st.sidebar.slider("種子 (Seeds) 數量", 10, 100, 50)
    k_candidates = st.sidebar.slider("候選池 (Candidates) 數量 / Seed", 10, 100, 40)
    n_distractors = st.sidebar.number_input("混入干擾項 (Distractors) 數量", value=500)
        ...
        if not st.session_state.seeds:
            ...
            with col_right:
                ...
                if not cand_data:
                    ...
                    # 每列顯示 4 張影像
                    cols_c = st.columns(4)
                    for c_idx, (cand_path, score) in enumerate(cand_data):
                        with cols_c[c_idx % 4]:
                            st.markdown("<div class='card'>", unsafe_allow_html=True)
                            ...
                            is_checked = st.checkbox(
                                "標記為 GT",
                                value=was_selected,
                                key=f"check_{selected_idx}_{c_idx}"
                            )
