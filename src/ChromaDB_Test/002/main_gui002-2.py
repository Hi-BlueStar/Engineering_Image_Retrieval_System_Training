...
from PySide6.QtGui import QPixmap, QColor, QDragEnterEvent, QDropEvent, QIcon, QFont, QPainter, QPainterPath
...
class ChromaManager:
    ...
    def is_target_image(self, filename: str, item_name: str) -> bool:
        """過濾邏輯"""
        if not filename.lower().endswith(".png"): return False
        if filename == f"{item_name}_merged.png": return True
        prefix = f"{item_name}_random_"
        if filename.startswith(prefix):
            try:
                suffix = filename[len(prefix):-4]
                num = int(suffix)
                if 1 <= num <= 20: return True
            except ValueError:
                pass
        return False
                ...
                try:
                    ...
                    for file in files:
                        ...
                        if progress_callback and processed % 10 == 0:
                            progress_callback(int((processed / total_files) * 50)) # 掃描佔 50%
...
class ResultCard(QFrame):
    ...
    def __init__(self, data):
        ...
        if os.path.exists(data['file_path']):
            pix = QPixmap(data['file_path']).scaled(100, 100, Qt.KeepAspectRatio, Qt.SmoothTransformation)
        ...
        sim_color = "#00ff88" if sim_val > 90 else "#ffcc00" if sim_val > 70 else "#ff5555"
        sim_lbl = QLabel(f"相似度: {sim_val:.1f}%")
        sim_lbl.setStyleSheet(f"color: {sim_color}; font-weight: bold; font-size: 13px;")
...
class SearchWorker(QThread):
    ...
    def run(self):
        ...
        stats_sorted = stats.sort_values(by='max_similarity', ascending=False).reset_index(drop=True)
...
class MainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        left_panel.setStyleSheet(f"background-color: {NeumorphicStyle.PANEL_COLOR}; border-radius: 20px;")
        # Shadow for panel
        l_shadow = QGraphicsDropShadowEffect()
        l_shadow.setBlurRadius(20); l_shadow.setColor(QColor(0,0,0,100)); l_shadow.setOffset(0,5)
        ...
        db_group_lbl.setStyleSheet("color: #00d4ff; font-weight: bold; margin-top: 20px;")
        ...
        self.results_grid = QVBoxLayout(self.scroll_content) # 使用 VBox 條列式，或 FlowLayout 網格
        ...
        # Search More Button
        self.btn_search_more = NeumorphicButton("🔍 找不到預期結果？ 搜尋更多...", is_primary=True)
    ...
    def on_search_finished(self, df):
        ...
        top_item = df.iloc[0]
        self.result_status.setText(f"最佳匹配: {top_item['item_name']} (信心: {top_item['max_similarity']:.2%})")
