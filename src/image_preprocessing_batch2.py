        ...
        def worker():
            while True:
                ...
                if ok:
                    # 顯示記憶體變化 (Pipeline 階段)
                    pipe_stats = next(
                        (s for s in profiler.results if s.name == "Pipeline"), None
                    )
                    mem_info = (
                        f"RSS:{_fmt_bytes(pipe_stats.rss_delta)}" if pipe_stats else ""
                    )
                    progress.update(
                        work_task,
                        advance=1,
                        description=f"完成: {img_path.name} [{mem_info}]",
                    )
                else:
                    progress.update(
                        work_task,
