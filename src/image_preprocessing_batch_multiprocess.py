        ...
        def wrapper(*args, **kwargs):
            ...
            if psutil:
                while True:
                    ...
                    if console:
                        # 顯示一個瞬態訊息 (不會保留在 log)
                        console.print(f"[yellow dim]系統 RAM 負載過高 ({mem.percent}%)，執行緒暫停等待中...[/]", end="\r")
                ...
                try:
                    ...
                    while True:
                        ...
                        if used_percent < gpu_mem_threshold:
                            break
                        if console:
                            console.print(f"[yellow dim]GPU 顯存負載過高 ({used_percent:.1f}%)，執行緒暫停等待中...[/]", end="\r")
                        time.sleep(check_interval)
                except Exception:
                    pass  # 獲取失敗則略過檢查
...
def _process_image_task(args: Tuple[Path, BatchConfig]) -> FileStat:
    ...
    if cfg.skip_existing and _already_done(out_dir, img_path.stem, suffix):
        return FileStat(
            ...
            error=None, output_dir=out_dir, saved_files=None
        )
    ...
    return FileStat(
        ...
        error=err_msg, output_dir=out_dir, saved_files=saved_files or None
    )
...
def process_folder(cfg: BatchConfig) -> Dict:
    ...
    console.print(f"[b]圖片數[/]: {len(files)}  | [b]程序數 (Processes)[/]: {num_procs}")
    ...
    def _build_report() -> Dict:
        ...
        return {
            "count": len(results), "ok": ok_cnt, "failed": fail_cnt,
            "output_root": str(Path(cfg.output_root).resolve()),
            "items": [
                {
                    "path": str(r.path), "ok": r.ok, "duration_sec": r.duration_sec,
                    "rss_delta": r.rss_delta, "py_peak_bytes": r.py_peak_bytes,
                    "error": r.error, "output_dir": str(r.output_dir) if r.output_dir else None,
                    "saved_files": r.saved_files,
                } for r in results
            ],
            "params": {
                "top_n": cfg.top_n, "remove_largest": cfg.remove_largest, "seed": cfg.seed,
                "padding": cfg.padding, "max_attempts": cfg.max_attempts, "random_count": cfg.random_count,
                "patterns": cfg.patterns, "recursive": cfg.recursive, "skip_existing": cfg.skip_existing,
                "max_workers": cfg.max_workers, "preserve_structure": cfg.preserve_structure,
            }
        }

    def _maybe_write_report(force: bool = False) -> None:
        if not cfg.write_report_json or not results: return
        if not force and len(results) % 5 != 0: return
    ...
    with progress:
        ...
        try:
            ...
            with mp.Pool(processes=num_procs) as pool:
                for res in pool.imap_unordered(_process_image_task, task_args, chunksize=1):
                    results.append(res)
    ...
    display_limit = 20
    display_results = results if len(results) <= display_limit else results[-display_limit:]
