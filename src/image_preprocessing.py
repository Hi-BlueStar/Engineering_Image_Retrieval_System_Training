...
class TopologyAnalyzerArray:
    ...
    def __init__(self, out_dir: Path | None = None):
        ...
        self._img_bin: np.ndarray | None = None  # 0/255
        self._img_skel: np.ndarray | None = None  # 0/255
    ...
    def run_once(
        self, bin01: np.ndarray, do_prune: bool = True
...
# ------------------------------------------------------------
def process_roi_iter(
    roi01: np.ndarray, iterations: int, out_root: Path | None
) -> np.ndarray:
    """
...
def run_pipeline(
    ...
    if not simplify_output:
        ...
        for comp in comps:
            ...
            cv2.imwrite(
                str(comp_dir / f"component_{comp.label}_area{comp.area}.png"), canvas
            )
    ...
    for L in large:
        ...
        if not simplify_output:
            cv2.imwrite(
                str(L_out / "0_original_mask.png"),
                (original_roi_mask * 255).astype(np.uint8),
            )
        ...
        if not simplify_output:
            cv2.imwrite(
                str(L_out / "pruned_skeleton.png"), (skel_roi * 255).astype(np.uint8)
            )
    ...
    for L in large:
        ...
        all_merged[y : y + h, x : x + w] = np.maximum(
            all_merged[y : y + h, x : x + w], roi
        )
    ...
    if not simplify_output:
        # ---- 全圖（黑底骨架） ----
        cv2.imwrite(str(merged_dir / "merged_all_large.png"), all_merged)
    ...
    if simplify_output:
        ...
        # 原始模式：儲存到 merged 資料夾
        cv2.imwrite(str(merged_dir / "merged_all_large_bgMatch.png"), colored)
    # --- [修改結束] ---
