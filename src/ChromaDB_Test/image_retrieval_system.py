    ...
    def scan(self) -> List[Dict[str, Any]]:
        ...
        for category in categories:
            ...
            for item_name in items:
                ...
                try: files_in_item = os.listdir(item_path)
    ...
    def _create_meta(self, path: str, category: str, item_name: str, img_type: str) -> Dict[str, Any]:
        return {
            ...
            "metadata": {
                "category": category,
                "item_name": item_name,
                "folder_path": os.path.dirname(path).replace("/large_components", ""), # 修正路徑以指向父層
...
class MultiModelDBManager:
    ...
    def ingest(self, data: List[Dict], embedding_fn: BaseEmbeddingFunction, batch_size=20):
        collection = self.get_collection(embedding_fn)
        # 檢查是否已經有資料，若有則簡單跳過 (實際生產環境可做增量更新)
        if collection.count() > 0:
            print(f"[Info] Collection {collection.name} already has {collection.count()} items. Skipping ingestion.")
            return
...
class SearchEngine:
    ...
    def search_image(self, 
        ...
        for emb_fn in embedding_fns:
            ...
            ).sort_values(by='max_sim', ascending=False).head(5) # 只取前 5 高的「名稱」
            ...
            for item_name, row in agg_df.iterrows():
                # 重組原始圖片路徑
                original_img_path = os.path.join(row['folder_path'], f"{item_name}_original.png")
