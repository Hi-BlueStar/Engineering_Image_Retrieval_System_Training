...
def search_and_aggregate(query_image_path: str, collection, top_k: int = 20):
    ...
    results = collection.query(
        ...
        n_results=top_k,
        include=['metadatas', 'distances'] # 我們需要 MetaData (取得名稱) 和 Distances (計算相似度)
    )
    ...
    for meta, dist in zip(metadatas, distances):
        ...
        parsed_data.append({
            ...
            "type": meta.get("type", "unknown"),  # 擷取圖片類型 (merged, random, large_component)
    ...
    stats = df.groupby('item_name').agg(
        ...
        primary_match_type=('type', lambda x: x.value_counts().index[0]), # 找出出現頻率最高的類型
    ...
    print(stats_sorted[['item_name', 'category', 'primary_match_type', 'count', 'max_similarity']].head(5))
    ...
    print(f"佐證數量:     {top_result['count']} / {top_k} (在前 {top_k} 筆結果中出現的次數)")
