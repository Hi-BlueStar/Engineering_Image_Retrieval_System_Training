...
class ChromaBackend:
    ...
    def add_images(self, file_paths):
        ...
        if ids:
            self.collection.add(ids=ids, uris=uris, metadatas=metadatas)
        return len(ids)
...
class ImageDropZone(QLabel):
    ...
    def mousePressEvent(self, event):
        file_path, _ = QFileDialog.getOpenFileName(
            self, "選取查詢圖片", "", "Images (*.png *.jpg *.jpeg *.bmp)"
        )
        if file_path:
    ...
    def __init__(self, data):
        ...
        if os.path.exists(img_path):
            ...
            self.img_label.setPixmap(
                pix.scaled(180, 160, Qt.KeepAspectRatio, Qt.SmoothTransformation)
            )
        else:
...
class EngineeringSearchApp(QMainWindow):
    ...
    def setup_ui(self):
        ...
        db_title.setStyleSheet(
            "font-size: 16px; font-weight: bold; color: #fff; margin-top: 10px;"
        )
        ...
        self.lbl_status.setStyleSheet(
            "font-size: 20px; font-weight: bold; color: #4db8ff;"
        )
    ...
    def start_search(self, reset=True):
        ...
        self.search_worker = SearchWorker(
            self.backend, self.current_query_path, self.current_k
        )
