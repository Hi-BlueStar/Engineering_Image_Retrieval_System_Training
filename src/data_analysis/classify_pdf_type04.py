...
def scan_directory(directory: str):
    ...
    with Progress(*progress_layout, console=console) as progress:
        ...
        for pdf_path in all_pdfs:
            ...
            if analysis["status"] == "success":
                ...
                # 統計低解析度 (僅針對有點陣圖的檔案)
                if analysis["dpi_stats"]["status"] == "Low Res" and (t == "Raster" or analysis["dpi_stats"]["avg"] > 0):
                    low_res_count += 1
            else:
                error_count += 1
    ...
    for res in results:
        ...
        if analysis["status"] == "success":
            ...
            table.add_row(path_str, type_style, size_page_str, pa['size'], dpi_display, date_str)
        else:
            err_msg = escape(analysis["msg"])
            table.add_row(path_str, "[bold red]ERROR[/bold red]", "-", "-", "-", f"[red]{err_msg}[/red]")
