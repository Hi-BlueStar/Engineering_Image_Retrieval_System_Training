...
class SimSiamEmbedding(BaseEmbeddingFunction):
    ...
    def __init__(self, model_path: str = None):
        ...
        if model_path and os.path.exists(model_path):
            ...
            print(
                "[Info] No custom SimSiam weights provided. Using ImageNet initialization."
            )
        ...
        for cat in categories:
            ...
            for item in items:
                ...
                try:
                    ...
                    for f in fs:
                        if f == f"{item}_merged.png":
                            add_file(f, "merged")
                        elif re.match(
                            rf"^{re.escape(item)}_random_(?:0[1-9]|1[0-9]|20)\.png$", f
                        ):
                            add_file(f, "random")
        ...
        for item_name, group in df.groupby("item_name"):
            if len(group) < 2:
                ...
                train_set, test_set = train_test_split(
                    group, test_size=test_size, random_state=42
                )
                gallery_list.extend(train_set.to_dict("records"))
                query_list.extend(test_set.to_dict("records"))

        print(
            f"[Split Info] Total: {len(df)} | Gallery (DB): {len(gallery_list)} | Query (Test): {len(query_list)}"
        )
        ...
        for emb_fn in embedding_fns:
            ...
            for i in tqdm(range(0, len(gallery_data), batch_size), leave=False):
                ...
                collection.add(
                    ...
                    metadatas=[
                        {"item_name": b["item_name"], "path": b["path"]} for b in batch
                    ],
