...
def _run_single_training(
    ...
    try:
            ...
            if labeled_ds is not None:
                if should_eval:
                    try:
                        ...
                        return eval_metrics.get("macro_mAP", None)
        ...
        return {
            ...
            "n_val": n_val,
            "status": "success",
            "log_dir": str(run_tracker.run_dir) if run_tracker is not None else "",
            **eval_metrics,
        }
        ...
        if run_tracker is not None:
            run_tracker.close(status="FINISHED" if run_status == "success" else "FAILED")
...
def main() -> None:
    ...
    # --- 設定 ---
    has_hydra_syntax = any("=" in arg and not arg.startswith("-") for arg in sys.argv[1:])
    is_custom_config = "--config" in sys.argv and not args.config.endswith("config.yaml")

    if is_custom_config and not has_hydra_syntax:
        logger.info("檢測到傳統設定檔路徑: %s，採用 from_yaml 載入", args.config)
        cfg = AppConfig.from_yaml(args.config, cli_overrides=overrides)
    else:
        logger.info("採用 Hydra 載入與組合設定...")
        hydra_overrides = []
        skip_next = False
        ...
        cfg = AppConfig.from_hydra(config_name="config", overrides=hydra_overrides)
